...
:::{.callout-note}
### Videos Teóricos
Los videos teóricos de esta unidad se encuentran en <a href="https://www.youtube.com/watch?v=kvljFoKc5ew&list=PLREanPs7CWgjUz9aF4O4T7KWvCznlsKUy" target="_blank">esta lista de reproducción</a>. Los videos son a modo introductorio: para la correcta aprobación de la materia se deben leer los apuntes.
:::
...
![Ejemplo de tabla](./imgs/unidad_6/pandas.png)

Pandas es una biblioteca un poco más pesada que otras por todo lo que incluye, por lo que vamos a poder aprovechar mucho más <a href="https://colab.research.google.com/" target="_blank">Google Colab</a>, que maneja grandes datos de forma rápida y eficiente.
...
El índice es de vital importancia ya que permite acceder a los elementos de la serie (muy parecido a cómo las claves nos permiten acceden a los valores de un diccionario, o el índice a un elemento de una lista). Al elegir índices personalizados, tenemos que tener en cuenta que su longitud debe ser acorde al número de elementos de la Serie. De lo contrario, se mostrará un ValueError.
...
Las Series no requieren recorrer valor por valor en un ciclo for si queremos realizar operaraciones. Podemos usar directamente operadores con Series.\
Por ejemplo:
...
| `.size` | Número de elementos, aunque también puede usarse `len` |
...
:::{.callout-note}
Muchas veces no vamos a querer modificar el dataframe original, pero sí vamos a querer manipularlo. En ese caso, podemos hacer una copia del dataframe original, de esta forma: `df_copy = df.copy()`
:::
...
| Aspecto | Archivos con `open()` | Archivos con Pandas |
|---------|----------------------|---------------------|
| **Memoria** | Podemos leer línea por línea (bajo consumo) | Carga todo en memoria (alto consumo para archivos grandes) |
| **Cerrar archivo** | Debemos cerrar manualmente o usar `with` | Se cierra automáticamente |
...
def copiar_notas_alumno(archivo_entrada, dni_buscado):
    ...
    df_filtrado.to_csv(ruta_salida, sep=';', index=False) # Esto es lo mismo que usar open(ruta_salida, "w") + writelines.
```
...
Esta parte del apunte es opcional, es decir, no se evalúa en los exámenes.
...
La grilla se puede agregar con al función `grid()`, y es una buena forma de darle legibilidad a un gráfico como puede ser el de línea y el de puntos.\
:::
...
![Los labels son muy grandes y no hay espacio para mostrarlos correctamente](./imgs/unidad_6/pbi_wrong.png)
...
![Los labels presentan un ángulo de 90 grados](./imgs/unidad_6/pbi_ok.png)
